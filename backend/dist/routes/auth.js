...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user && ALLOWED_USER_ROLES.includes(String(user.role).toUpperCase())) {
        ...
        if (user && passwordValid && ALLOWED_USER_ROLES.includes(String(user.role).toUpperCase())) {
...
router.post('/users', authMiddleware, requireRoles(['ADMIN']), requireCsrfToken, async (req, res) => {
    ...
    if (!email || !password || !ALLOWED_USER_ROLES.includes(role)) {
...
router.delete('/users/:id', authMiddleware, requireRoles(['ADMIN']), requireCsrfToken, async (req, res) => {
    const id = sanitizeLooseText(req.params?.id);
    if (!id)
        return res.status(400).json({ message: 'Invalid user id' });
